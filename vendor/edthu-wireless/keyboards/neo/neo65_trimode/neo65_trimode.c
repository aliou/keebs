...
uint32_t led_blink_callback(uint32_t trigger_time, void *cb_arg) {
    ...
    uint8_t bit = 1 << phase;
    gpio_write_pin(ESCAPE_PIN, (pattern[led_blink_state[DEVS_USB]] & bit) != 0);
    gpio_write_pin(DEVS_BT1_PIN, (pattern[led_blink_state[DEVS_BT1]] & bit) != 0);
    gpio_write_pin(DEVS_BT2_PIN, (pattern[led_blink_state[DEVS_BT2]] & bit) != 0);
    gpio_write_pin(DEVS_BT3_PIN, (pattern[led_blink_state[DEVS_BT3]] & bit) != 0);
    gpio_write_pin(DEVS_2G4_PIN, (pattern[led_blink_state[DEVS_2G4]] & bit) != 0);
